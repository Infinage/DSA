...
struct TreeNode {
     ...
     */
    static TreeNode* from(std::initializer_list<T> array, const T &null) {
        // Convert into an array of TreeNodes
        std::vector<TreeNode*> forest(array.size());
        std::transform(array.begin(), array.end(), forest.begin(), 
            [&null](const T &val) { 
                return val == null? nullptr: new TreeNode {val}; 
            }
        );

        // Link the TreeNode to parent
     ...
     */
    static std::vector<T> toVector(TreeNode *root, const T &null) {
        std::vector<T> res; std::queue<TreeNode<T>*> queue; queue.push(root);
        while (!queue.empty()) {
