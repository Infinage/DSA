...
<div id="c58685d3" class="cell markdown">

# 73.

Date: 02nd Feb 2024<br> Medium Leetcode:
all-paths-from-source-to-target<br> Solution:
<https://leetcode.com/problems/all-paths-from-source-to-target/submissions/1163989706/>

</div>

<div id="70baf749" class="cell code" execution_count="75">

``` python
class Solution:
    def allPathsSourceTarget(self, graph: List[List[int]]) -> List[List[int]]:
        # Create an adjacency list and use a DFS algorithm to fund all paths
        graph = dict(enumerate(graph))
        target = len(graph) - 1

        result = []
        def backtrack(path: list[int] = [0]):
            curr = path[-1]
            if curr == target:
                result.append(path)
            else:
                for next_ in graph[curr]:
                    if next_ not in path:
                        backtrack([*path, next_])
        
        backtrack()
        return result

# Testing the solution
assert sorted(Solution().allPathsSourceTarget([[1,2],[3],[3],[]])) == sorted([[0,1,3],[0,2,3]])
assert sorted(Solution().allPathsSourceTarget([[4,3,1],[3,2,4],[3],[4],[]])) == sorted([[0,4],[0,3,4],[0,1,3,4],[0,1,2,3,4],[0,1,4]])
assert sorted(Solution().allPathsSourceTarget([[1],[]])) == sorted([[0,1]])
```

</div>

<div id="89278b41" class="cell markdown">

# 74.

Date: 02nd Feb 2024<br> Medium Leetcode:
rearrange-array-elements-by-sign<br> Solution:
<https://leetcode.com/problems/rearrange-array-elements-by-sign/submissions/1164036105>

</div>

<div id="2cea39d6" class="cell code" execution_count="76">
...
<div id="efd6f399" class="cell markdown">

# 75.

Date: 02nd Feb 2024<br> Medium Leetcode:
partition-array-according-to-given-pivot<br> Solution:
<https://leetcode.com/problems/partition-array-according-to-given-pivot/submissions/1164120125/>

</div>

<div id="ce06d4e9" class="cell code" execution_count="77">
...
class Solution:
    ...
    def pivotArray_(self, nums: List[int], pivot: int) -> List[int]:
        "Using O(1) space complexity, time complexity is O(N ^ 2)"

        def shift_right(start, curr):
            'Shift all elements starting at start right and insert curr at start'
            temp = nums[curr]
            while curr > start:
...
# Testing the solution
assert Solution().pivotArray([2,4,3,-3], 2) == [-3,2,4,3]
